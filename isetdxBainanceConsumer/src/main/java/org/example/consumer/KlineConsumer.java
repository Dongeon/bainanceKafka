...
public class KlineConsumer implements Runnable, AutoCloseable {
    ...
    public KlineConsumer(AppConfig config, KlineRepository repository) {
        this.repository = repository;
        this.topic      = config.klineTopic();

        Properties props = new Properties();
        props.put(ConsumerConfig.BOOTSTRAP_SERVERS_CONFIG,        config.bootstrapServers());
        props.put(ConsumerConfig.GROUP_ID_CONFIG,                 config.klineGroupId());
    ...
    private void process(ConsumerRecord<String, String> record) {
        try {
            ...
            if (count % STATUS_INTERVAL == 0) {
                log.info("[KLINE] 누적 {}건 | {}@{} close={}", count,
                        event.getSymbol(), event.getInterval(), event.getClosePrice());
            }
        } catch (Exception e) {
            log.error("Kline 처리 실패 [partition={} offset={}]: {}",
